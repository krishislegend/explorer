  ...
  find_block(number, function(block_db) {
    if (block_db) {
      ...
      if (block.transactions) {
        module.exports.create_wantxs(block.transactions, block.timestamp, function(err) {
          if (err) {
            console.log(err);
            return cb(err);
          } else {
...
function save_wantx(tx, block_timestamp, cb) {
  // Check if address exists
  ...
  find_tx(tx.hash, function(tx_db) {
    if (tx_db) {
      ...
      >>>>>>>714 f4ae9124e330204aad9fd90e060f790f246c6
...
module.exports = {
  ...
  //WZ
  create_wantx: function(tx, timestamp, cb) {
    save_wantx(tx, timestamp, function(err) {
      if (err) {
        return cb(err);
      } else {
  ...
  create_wantxs: function(block_txs, timestamp, cb) {
    lib.syncLoop(block_txs.length, function(loop) {
      var i = loop.iteration();
      save_wantx(block_txs[i], timestamp, function(err) {
        if (err) {
          loop.next();
        } else {
