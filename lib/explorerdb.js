...
function update_wanaddress(hash, txid, amount, type, balances, iscontract, cb) {
...
function update_block(number, block, balances, isContracts, cb) {
  // Check if address exists
  ...
  find_block(number, function(block_db) {
    if (block_db) {
      ...
      if (block.transactions) {
        module.exports.create_wantxs(block.transactions, block.timestamp, balances, isContracts, function(err) {
          if (err) {
            console.log(err);
            return cb(err);
          } else {
...
function save_wantx(tx, block_timestamp, balances, isContracts, cb) {
  // Check if address exists
  ...
  find_tx(tx.hash, function(tx_db) {
    if (tx_db) {
      ...
      var to = tx.to;
      if (!to || 0 === to.length) {
        tx.to = isContracts[tx.hash][0];
        tx.createContract = isContracts[tx.hash][1];
      }
      ...
      newtx.save(function(err) {
        if (err) {
          ...
          update_wanaddress(tx.from, tx.hash, 1, 'send', balances, isContracts[tx.hash][1], function(err) {
            console.log(err);
            if (err) {
              return cb(err);
            } else {
              ...
              update_wanaddress(tx.to, tx.hash, 1, 'receive', balances, isContracts[tx.hash][1], function(err) {
                if (err) {
                  return cb(err);
                } else {
...
module.exports = {
  ...
  update_block: function(number, block, balances, isContracts, cb) {
    console.log("calling update_block");
    update_block(number, block, balances, isContracts, function(outblock) {
      return cb(outblock);
    });
  },
  ...
  create_wantxs: function(block_txs, timestamp, balances, isContracts, cb) {
    lib.syncLoop(block_txs.length, function(loop) {
      var i = loop.iteration();
      save_wantx(block_txs[i], timestamp, balances, isContracts, function(err) {
        if (err) {
          loop.next();
        } else {
